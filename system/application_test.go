...
func testApplication(t *testing.T, context spec.G, it spec.S) {
	...
	it.Before(func() {
		...
		application, err = system.NewApplication(ctx.Application.Path, "test-command",
			[]string{"test", "default", "arguments"}, "*.[jw]ar")
		Expect(err).NotTo(HaveOccurred())
	...
	it("contributes layer", func() {
		...
		Expect(e.Args).To(Equal([]string{"test", "default", "arguments"}))
	...
	context("ResolveArguments", func() {
		it("uses default arguments", func() {
			Expect(application.ResolveArguments()).To(Equal([]string{"test", "default", "arguments"}))
		})

		context("$BP_BUILD_ARGUMENTS", func() {

			it.Before(func() {
				Expect(os.Setenv("BP_BUILD_ARGUMENTS", "test configured arguments")).To(Succeed())
			})

			it.After(func() {
				Expect(os.Unsetenv("BP_BUILD_ARGUMENTS")).To(Succeed())
			})
	...
	context("ResolveArtifact", func() {
		it("fails with no files", func() {
			...
			Expect(err).To(MatchError("unable to find built artifact (executable JAR or WAR) in *.[jw]ar, candidates: []"))
		})
		...
		it("fails with multiple candidates", func() {
			for _, f := range []string{"stub-application.jar", "stub-application.war", "stub-executable.jar"} {
				...
				out, err := os.OpenFile(filepath.Join(ctx.Application.Path, f), os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
				Expect(err).NotTo(HaveOccurred())
			...
			Expect(err).To(MatchError(
				fmt.Sprintf("unable to find built artifact (executable JAR or WAR) in *.[jw]ar, candidates: [%s %s %s]",
					filepath.Join(ctx.Application.Path, "stub-application.jar"),
					filepath.Join(ctx.Application.Path, "stub-application.war"),
					filepath.Join(ctx.Application.Path, "stub-executable.jar"))))
		...
		it("passes with a single candidate", func() {
			...
			Expect(application.ResolveArtifact()).To(Equal(filepath.Join(ctx.Application.Path, "stub-application.jar")))
		})
		...
		it("passes with a single executable JAR", func() {
			...
			Expect(application.ResolveArtifact()).To(Equal(filepath.Join(ctx.Application.Path, "stub-executable.jar")))
		})
		...
		it("passes with a single WAR", func() {
			...
			Expect(application.ResolveArtifact()).To(Equal(filepath.Join(ctx.Application.Path, "stub-application.war")))
		})

		context("$BP_BUILT_MODULE", func() {

			it.Before(func() {
				Expect(os.Setenv("BP_BUILT_MODULE", "test-directory")).To(Succeed())
			})

			it.After(func() {
				Expect(os.Unsetenv("BP_BUILT_MODULE")).To(Succeed())
			})

			it("passes with $BP_BUILT_MODULE", func() {
		...
		context("$BP_BUILT_ARTIFACT", func() {
			it.Before(func() {
				Expect(os.Setenv("BP_BUILT_ARTIFACT", "test-directory/stub-application.jar")).To(Succeed())
			})

			it.After(func() {
				Expect(os.Unsetenv("BP_BUILT_ARTIFACT")).To(Succeed())
			})

			it("passes with BP_BUILT_ARTIFACT", func() {
				Expect(os.MkdirAll(filepath.Join(ctx.Application.Path, "test-directory"), 0755)).To(Succeed())
